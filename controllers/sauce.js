...
exports.feedbackSauce = (req, res, next) => {
  if (req.body.like === 1) {
    Sauce.updateOne(
      ...
      .then(() => res.status(200).json({ message: "Pouce vert !" }))
      .catch((error) => res.status(400).json({ error }));
    ...
    Sauce.updateOne(
      ...
      .then(() => res.status(200).json({ message: "Pouce Rouge !" }))
      .catch((error) => res.status(400).json({ error }));
      ...
      .then((sauce) => {
        if (sauce.usersLiked.includes(req.body.userId)) {
          Sauce.updateOne(
            ...
            .then(() => {
              res.status(200).json({ message: "Plus de like !" });
            })
            .catch((error) => res.status(400).json({ error }));
          ...
          Sauce.updateOne(
            ...
            .then(() => {
              res.status(200).json({ message: "Plus de dislike !" });
            })
            .catch((error) => res.status(400).json({ error }));
        }
      })
